...
public interface MagicpileEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/magicpile */
    ...
    /** The key of the configuration. e.g. jdbc:mysql://localhost:3306/magicpiledb */
    String JDBC_URL = "jdbc.url";

    /** The key of the configuration. e.g. magicpiledb */
     ...
     * The value is, e.g. /tmp/lastaflute/magicpile <br>
     ...
     * The value is, e.g. jdbc:mysql://localhost:3306/magicpiledb <br>
     ...
     * The value is, e.g. magicpiledb <br>
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements MagicpileEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(MagicpileEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(MagicpileEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(MagicpileEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(MagicpileEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(MagicpileEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(MagicpileEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(MagicpileEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(MagicpileEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(MagicpileEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(MagicpileEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(MagicpileEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(MagicpileEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(MagicpileEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(MagicpileEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(MagicpileEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(MagicpileEnv.MAIL_RETURN_PATH);
        }

        public String getJdbcDriver() {
            return get(MagicpileEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(MagicpileEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(MagicpileEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(MagicpileEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(MagicpileEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(MagicpileEnv.JDBC_CONNECTION_POOLING_SIZE);
        }
    }
}
