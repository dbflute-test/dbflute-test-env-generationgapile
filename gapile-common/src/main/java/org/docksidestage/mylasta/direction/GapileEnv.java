...
public interface GapileEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/gapile */
    ...
    /** The key of the configuration. e.g. jdbc:mysql://localhost:3306/gapiledb */
    String JDBC_URL = "jdbc.url";

    /** The key of the configuration. e.g. gapiledb */
     ...
     * The value is, e.g. /tmp/lastaflute/gapile <br>
     ...
     * The value is, e.g. jdbc:mysql://localhost:3306/gapiledb <br>
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements GapileEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(GapileEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(GapileEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(GapileEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(GapileEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(GapileEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(GapileEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(GapileEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(GapileEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(GapileEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(GapileEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(GapileEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(GapileEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(GapileEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(GapileEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(GapileEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(GapileEnv.MAIL_RETURN_PATH);
        }

        public String getJdbcDriver() {
            return get(GapileEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(GapileEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(GapileEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(GapileEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(GapileEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(GapileEnv.JDBC_CONNECTION_POOLING_SIZE);
        }
    }
}
